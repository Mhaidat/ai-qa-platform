...
Stores links between platform artifacts and Jira issues.
...
| ----------------- | ------------------------ | -------: | --------------------------------- |
| Id                | uuid                     |       No | Primary key                       |
| JiraIntegrationId | uuid                     |       No | Related Jira Integration          |
| EntityType        | varchar(30)              |       No | Platform artifact type            |
| EntityId          | uuid                     |       No | Platform artifact identifier      |
| JiraIssueId       | varchar(100)             |       No | Jira internal issue identifier    |
| JiraIssueKey      | varchar(100)             |       No | Jira issue key                    |
| JiraIssueType     | varchar(100)             |      Yes | Jira issue type                   |
| JiraUrl           | text                     |      Yes | Direct Jira issue URL             |
| SyncStatus        | varchar(30)              |       No | Synchronization status            |
| LastSyncedAt      | timestamp with time zone |      Yes | Latest successful synchronization |
| LastSyncError     | text                     |      Yes | Latest synchronization failure    |
| CreatedByUserId   | uuid                     |       No | User who created the Link         |
| CreatedAt         | timestamp with time zone |       No | Creation timestamp                |
| UpdatedAt         | timestamp with time zone |      Yes | Last update timestamp             |
| DeletedAt         | timestamp with time zone |      Yes | Soft deletion timestamp           |
...
* `JiraIntegrationId, EntityType, EntityId` for active Links
* `JiraIntegrationId, JiraIssueId` for active Links
...
* A Jira Link belongs to exactly one Jira Integration.
* A Jira Link references one platform artifact.
* The linked artifact must belong to the same Project as the Jira Integration.
* Jira synchronization is initiated manually.
* Removing a Jira Link does not delete the platform artifact or Jira issue.
* Failed synchronization does not remove an existing valid Link.
* Jira identifiers are external references and must never be used as platform primary keys.
* Soft-deleted Links are excluded from active synchronization.
...
| ---------------- | ------------------------ | -------: | ------------------------------------------ |
| Id               | uuid                     |       No | Primary key                                |
| StoryId          | uuid                     |       No | Originating Story                          |
| AIAnalysisId     | uuid                     |      Yes | AI Analysis used to generate the Test Case |
| Source           | varchar(30)              |       No | Test Case source                           |
| Title            | varchar(500)             |       No | Test Case title                            |
| Description      | text                     |      Yes | Test Case description                      |
| Preconditions    | text                     |      Yes | Required conditions before execution       |
| TestData         | text                     |      Yes | General Test Case data                     |
| ExpectedResult   | text                     |      Yes | Overall expected result                    |
| Type             | varchar(50)              |       No | Test classification                        |
| Priority         | varchar(30)              |       No | Test priority                              |
| Status           | varchar(30)              |       No | Review and approval status                 |
| CreatedByUserId  | uuid                     |      Yes | User who manually created the Test Case    |
| ApprovedByUserId | uuid                     |      Yes | User who approved the Test Case            |
| ApprovedAt       | timestamp with time zone |      Yes | Approval timestamp                         |
| CreatedAt        | timestamp with time zone |       No | Creation timestamp                         |
| UpdatedAt        | timestamp with time zone |      Yes | Last update timestamp                      |
| ArchivedAt       | timestamp with time zone |      Yes | Archive timestamp                          |
...
* Every Test Case belongs to exactly one Story.
* The Test Case and Story must belong to the same Project.
* AI-generated Test Cases reference an `AIAnalysisId`.
* Manually created Test Cases reference `CreatedByUserId`.
* AI-generated Test Cases are never auto-approved.
* Only authorized Project Members can review or approve Test Cases.
* Approved Test Cases become visible in the Automation Workspace.
* A Test Case cannot be used for automation before approval.
* Updating a Test Case does not change its original Source.
* Rejected Test Cases remain stored for traceability.
* Soft-deleted Test Cases are excluded from active workflows.
...
Stores ordered execution steps belonging to a Test Case.
...
| -------------- | ------------------------ | -------: | ------------------------------- |
| Id             | uuid                     |       No | Primary key                     |
| TestCaseId     | uuid                     |       No | Owning Test Case                |
| StepNumber     | integer                  |       No | Execution order                 |
| Action         | text                     |       No | Action performed by the tester  |
| TestData       | text                     |      Yes | Data used in this Step          |
| ExpectedResult | text                     |      Yes | Expected result after this Step |
| CreatedAt      | timestamp with time zone |       No | Creation timestamp              |
| UpdatedAt      | timestamp with time zone |      Yes | Last update timestamp           |
...
* `TestCaseId` references `TestCases.Id`
...
* Every Test Step belongs to exactly one Test Case.
* `StepNumber` must be greater than zero.
* Step numbers must be unique within the same Test Case.
* Test Steps are returned in ascending `StepNumber` order.
* Deleting a Test Case deletes its Test Steps.
* A Draft Test Case may temporarily contain no Test Steps.
* An approved Test Case must contain at least one Test Step unless it is explicitly marked as scenario-only.
* Reordering Steps must update Step numbers consistently.

---

# AutomationGenerations Schema
...
Stores Playwright TypeScript generation requests and generated outputs.

Each generation attempt creates a new record.
...
| ------------------------ | ------------------------ | -------: | ----------------------------------------------- |
| Id                       | uuid                     |       No | Primary key                                     |
| TestCaseId               | uuid                     |       No | Approved Test Case used as input                |
| RequestedByUserId        | uuid                     |       No | Automation QA Engineer who requested generation |
| Status                   | varchar(30)              |       No | Generation execution status                     |
| Framework                | varchar(50)              |       No | Automation framework                            |
| Language                 | varchar(50)              |       No | Programming language                            |
| ModelProvider            | varchar(100)             |       No | AI provider                                     |
| ModelName                | varchar(150)             |       No | AI model                                        |
| PromptVersion            | varchar(50)              |       No | Prompt template version                         |
| GeneratedTestCode        | text                     |      Yes | Generated test code                             |
| GeneratedPageObjectCode  | text                     |      Yes | Generated Page Object code                      |
| GeneratedSupportingFiles | jsonb                    |      Yes | Metadata or content for supporting files        |
| PackageStoragePath       | text                     |      Yes | Downloadable ZIP storage path                   |
| InputTokenCount          | integer                  |      Yes | Input token usage                               |
| OutputTokenCount         | integer                  |      Yes | Output token usage                              |
| EstimatedCost            | numeric(18,6)            |      Yes | Estimated AI cost                               |
| ErrorCode                | varchar(100)             |      Yes | Structured failure code                         |
| ErrorMessage             | text                     |      Yes | Generation failure details                      |
| StartedAt                | timestamp with time zone |      Yes | Generation start                                |
| CompletedAt              | timestamp with time zone |      Yes | Generation completion                           |
| DownloadedAt             | timestamp with time zone |      Yes | First package download                          |
| CreatedAt                | timestamp with time zone |       No | Record creation timestamp                       |
| UpdatedAt                | timestamp with time zone |      Yes | Last update timestamp                           |
...
* Automation is generated only from an approved Test Case.
* The requesting User must be an active Automation QA Engineer, QA Lead, or Project Admin.
* The requesting User and Test Case must belong to the same Project and Organization.
* Every generation attempt creates a new record.
* Previous generated outputs are never overwritten.
* The MVP supports Playwright TypeScript only.
* Generated code must remain traceable to the original Test Case and Story.
* Complete ZIP packages are stored outside PostgreSQL.
* Failed generations remain stored for troubleshooting.
* Token usage and estimated AI cost should be recorded when available.
* Generated code must be reviewed before being considered production-ready.
* Downloading a package does not change the Test Case approval status.

---

# Test and Automation Referential Rules

## Deletion Behavior

* Deleting a Story must not physically delete approved Test Cases automatically.
* Deleting a Test Case should use soft deletion.
* Test Steps may use cascade deletion when their Test Case is permanently removed through controlled maintenance.
* Jira Links must remain traceable when a linked artifact is archived.
* Automation Generations must not be physically deleted when a Test Case is edited or archived.
* Historical generation records must remain available for auditing and cost tracking.

## Approval Rules

* Only approved Stories can be used to generate Test Cases.
* Only approved Test Cases can be used to generate Automation.
* AI output never advances a workflow automatically.
* Approval always requires an authorized human User.

## Traceability Chain

The platform must preserve the following chain:
...
# Remaining Database Review Items

Before generating Entity Framework Core entities, the following items must be reviewed:
